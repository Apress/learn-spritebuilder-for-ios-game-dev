...
#pragma message "Note: NSPointerArray was replaced by NSMutableArray because the cocos2d beta version freezes upon setPointer:(void*)self on Android"
	
	// get or create the list of triggers
	NSMutableArray* triggers = [triggerArrays objectForKey:_name];
	if (triggers == nil)
	{
...
-(void) addTargetsTo:(NSMutableArray*)targets searchInNode:(CCNode*)node
{
	...
	for (CCNode* child in node.children)
	{
		if ([child.name isEqualToString:_name])
		{
			...
			if ([child conformsToProtocol:@protocol(TriggerDelegate)])
			{
				...
				// if the target has a physics body, make it sleep - that way it can be activated by trigger
				child.physicsBody.sleeping = YES;
				child.paused = YES;
			}
		}
...
-(void) triggerActivatedBy:(CCNode*)activator
{
	...
	NSMutableArray* targets = [targetArrays objectForKey:_name];
	
	// send the trigger method to each target
	for (CCNode* target in targets)
	{
		// wake up physics body
		target.physicsBody.sleeping = NO;
		target.paused = NO;
		
		// TriggerDelegate is assumed as implemented due to the check in addTargetsTo:
		[(CCNode<TriggerDelegate>*)target didTriggerWithNode:activator];
	}
...
-(void) updateTriggers:(NSMutableArray*)triggers
{
...
-(void) cleanupTriggers:(NSMutableArray*)triggers
{
