...
-(void) updateTextureCoordinates
{
	...
	for (int i = 0; i < _numVertices; i++)
	{
		...
		//NSLog(@"pos: %.1f, %.1f, v: %.3f, %.3f", pos.x, pos.y, _vertices[i].texCoord1.x, _vertices[i].texCoord1.x);
	}
...
{
	BOOL existingFrameWasReplaced = (self.spriteFrame != nil);
	[super setSpriteFrame:spriteFrame];

	if (existingFrameWasReplaced)
	{
		// if the sprite frame changes (except for the first time) update the vertices and tex coords
		[self updateTextureCoordinates];
	}
}
*/
